...
CREATE POLICY IF NOT EXISTS "Users can view own quotas"
ON public.analysis_quotas
FOR SELECT
USING (auth.uid() = user_id);

-- Les utilisateurs peuvent insérer leurs propres quotas
CREATE POLICY IF NOT EXISTS "Users can insert own quotas"
ON public.analysis_quotas
FOR INSERT
WITH CHECK (auth.uid() = user_id);

-- Les utilisateurs peuvent mettre à jour leurs propres quotas
CREATE POLICY IF NOT EXISTS "Users can update own quotas"
ON public.analysis_quotas
FOR UPDATE
USING (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Users can insert own connections"
ON public.websocket_connections
FOR INSERT
WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Users can view own game analyses"
...
CREATE POLICY IF NOT EXISTS "Users can insert own game analyses"
ON public.game_analyses
FOR INSERT
WITH CHECK (
    auth.uid() = user_id 
...
CREATE POLICY IF NOT EXISTS "Users can update own analytics"
ON public.user_analytics
FOR UPDATE
USING (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Anyone can view tournament participants"
ON public.tournament_participants
FOR SELECT
USING (true);

-- Les utilisateurs peuvent s'inscrire eux-mêmes
CREATE POLICY IF NOT EXISTS "Users can register themselves"
ON public.tournament_participants
FOR INSERT
WITH CHECK (auth.uid() = user_id);

-- Les utilisateurs peuvent se désinscrire
CREATE POLICY IF NOT EXISTS "Users can unregister themselves"
ON public.tournament_participants
FOR DELETE
USING (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Anyone can view tournaments"
ON public.tournaments
FOR SELECT
USING (true);

-- Les utilisateurs peuvent créer des tournois
CREATE POLICY IF NOT EXISTS "Users can create tournaments"
ON public.tournaments
FOR INSERT
WITH CHECK (auth.uid() = created_by);

-- Les créateurs peuvent mettre à jour leurs tournois
CREATE POLICY IF NOT EXISTS "Creators can update own tournaments"
ON public.tournaments
FOR UPDATE
USING (auth.uid() = created_by);

-- Les créateurs peuvent supprimer leurs tournois
CREATE POLICY IF NOT EXISTS "Creators can delete own tournaments"
ON public.tournaments
FOR DELETE
USING (auth.uid() = created_by);
