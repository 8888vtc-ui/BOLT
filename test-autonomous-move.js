// Script de test autonome pour le déplacement des pions
// À injecter dans la console du navigateur

(function autonomousMoveTest() {
    ...
    // Fonction pour obtenir les mouvements légaux (via window ou console)
    function getValidMoves() {
        // Essayer d'accéder à la logique de jeu via window
        if (window.__GAME_STATE__) {
            return window.__GAME_STATE__.legalMoves || [];
        }
        
        // Sinon, essayer de trouver les points highlightés
    ...
    function selectLightChecker() {
        ...
        const clickHandler = () => {
            clickReceived = true;
            console.log('✅ CLICK REÇU SUR LE CHECKER');
        };
        
        firstChecker.addEventListener('click', clickHandler, { once: true });
        ...
        setTimeout(() => {
            if (!clickReceived) {
                console.warn('⚠️ Click non reçu, inspection des overlays...');
                checkOverlays();
                // Réessayer
                firstChecker.click();
            }
    ...
    async function runTest() {
        ...
        try {
            ...
            // Essayer les points 1-6 (zone de départ light)
